...
- **安全パターンの自動修正** — 名前定義修正・固定参照置換・数式範囲拡張・
  空セルへの説明追加に限定して、チャットで影響を試算 → ボタンで適用 → 予定差分と実差分を
  policy gate で厳密照合。適用は新しいジョブとして保存され、元ファイルは変更されない
- **変更手段の差し替え** — 変更計画を `MutationProvider` 境界で実行し、変更した
  道具の終了コードを安全性の根拠にしない。組み込み openpyxl のほか、任意の
  OfficeCLI アダプター（`.xlsx` の名前定義変更・空セルへの説明追加）を利用できる。変更計画・
  期待差分・実差分・判定は監査レコードとして保存される
- **VBA変更パッケージ** — 既存の `Sub` / `Function` 1件を丸ごと置換する計画から、
  原本 `.xlsm`、置換コード、計画JSON、Windows PowerShell実行スクリプトをZIP生成する。
  Windows Excel/VBIDEでコピーへ適用し、戻した `.xlsm` が期待VBA差分だけかを検証する。
  この段階ではVBAのコンパイル・実行結果までは保証しない
...
| `OFFICECLI_BIN` | PATH 検索 | 任意の OfficeCLI 実行ファイル。変更プロバイダーとして使う場合のみ指定 |
