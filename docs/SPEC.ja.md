...
5. アプリが**安全ゲート**（構造差分・危険箇所・再現テスト・ロールバック用コピー）を提示し、ユーザーが検収する
6. 検収された変更のうち安全なパターン（名前定義修正・固定参照置換・数式範囲拡張など）は、アプリが段階的に自動適用する。VBAを含む広範な自動修正は将来段階

> 5・6 は本書のロードマップ上の次段階 (P1: 安全ゲート/検収レポート)。現時点の実装は
> ステップ4まで (P0)。段階分け・実現可能性の検証結果・根拠は
...
Web UI 上でのこのユーザーの仕事は、ツールが構造的に適用できる変更については
