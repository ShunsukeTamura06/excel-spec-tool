"""POST /jobs/{job_id}/named-range-fix — 名前定義修正を実ファイルに適用し、
新ジョブを作って自己検証する (S2 増分1)。

人間が画面のボタンを押した時だけ呼ばれる。LLM の tool loop からは呼ばれない
...
from core.exceptions import DiffError, ExtractionError, NamedRangeFixError
...
def _apply_and_extract(
    ...
    # その後 apply_named_range_fix で新ジョブの original.* を書き換える。
    new_meta = storage.create_job(filename, data)
    new_path = storage.get_original_path(new_meta.job_id)
    apply_named_range_fix(source_path, name, new_refers_to, new_path)
...
async def apply_named_range_fix_route(
    ...
    try:
        new_job_id = await asyncio.to_thread(
            _apply_and_extract, storage, job_id, meta.filename, body.name, body.new_refers_to
        )
